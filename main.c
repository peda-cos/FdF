...
{
	mlx_instance->mlx = mlx_init(g_resolution_x, g_resolution_y, "FDF Viewer", true);
	if (!mlx_instance->mlx)
	{
		ft_putstr_fd("Error: Failed to initialize MLX42.\n", 2);
		free(mlx_instance->map);
		exit(EXIT_FAILURE);
	}

	mlx_instance->img = mlx_new_image(mlx_instance->mlx, g_resolution_x, g_resolution_y);
	if (!mlx_instance->img)
	{
